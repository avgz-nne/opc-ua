...
async def main():
    ...
    server = Server()
    await server.init()
    server.set_endpoint('opc.tcp://192.168.0.119:4840/nne_unibio/server/') # NOOOO new address here should be 192.168.10.10
    ...
    async with server:
        ...
        myobj2 = await server.nodes.objects.add_object(idx, 'POZYX')
